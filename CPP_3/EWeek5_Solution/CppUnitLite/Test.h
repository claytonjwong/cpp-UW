...
template <typename T, typename U> void check_equal_compare_ref(const T& expected, const U& actual, TestResult& result, const char* file, int line)
{
    if (!((expected) == (actual)))
    {
        std::ostringstream message;
...
#define CHECK_EQUAL_REF(expected, actual)\
{\
    check_equal_compare_ref(expected, actual, rEsUlT_, __FILE__, __LINE__); \
}
