...
namespace
{
	...
	VG::VectorGraphic parseVectorGraphic(deque<string>& theNodes)
	{
        ...
        //Non-const lvalue reference to type 'std::istream' (aka 'basic_istream<char>') cannot bind to a temporary of type 'std::__1::stringstream' (aka 'basic_stringstream<char>')
		//auto closed{ Parse::getBool(stringstream(match[2])) };
        stringstream temp{ match[2] };
        auto closed{ Parse::getBool(temp) };
